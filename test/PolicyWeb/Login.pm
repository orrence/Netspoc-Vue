...
sub test_load {
    my ($browser) = @_;
    $browser->get('/netspocvue/index.html');
    sleep 1;
...
sub test_find_login {
    my ($browser) = @_;
    ok( 1 == scalar @{ $browser->find_elements('//input[@id="txt_login"]') },
        'Found input text login' );
    ok(
        1 == scalar @{ $browser->find_elements('//input[@id="txt_password"]') },
        'Found input text password'
    );
    ok( 1 == scalar @{ $browser->find_elements('//button[@id="btn_login"]') },
        'Found button login' );
}
...
sub test_find_top_buttons {
    my ($browser) = @_;
    ok( 1 == scalar @{ $browser->find_elements('//a[@id="tab_services"]') },
        'Found button services tab' );
    ok( 1 == scalar @{ $browser->find_elements('//a[@id="tab_networks"]') },
        'Found button networks tab' );
    ok( 1 == scalar @{ $browser->find_elements('//a[@id="tab_diff"]') },
        'Found button differences tab' );
    ok(
        1 == scalar @{ $browser->find_elements('//input[@id="combo_policy"]') },
        'Found combo box policy'
    );
    ok(
        1 == scalar @{ $browser->find_elements('//input[@id="combo_owner"]') },
        'Found combo box owner'
    );
    ok(
        1 == scalar @{ $browser->find_elements('//button[@id="btn_logout"]') },
        'Found button logout'
    );
}
